...
class ChatBot:
  ...
  initiate_conversation = ("yes", "okay", "sure", "of course", "Yes")
  exit_commands = ("quit", "pause", "exit", "goodbye", "bye", "later", "stop")

  
  def start_chat(self):
    user_response = input("Hi, I'm a chatbot trained on dialog from Twitter posts about weather. Would you like to chat with me?\n")
    
    if user_response in self.negative_responses:
      print("Ok, have a great day!")
      return
    elif user_response in self.initiate_conversation:
      initiate = input("Great! Do you have a question?\n")
      self.chat(initiate)
    
    return
  
  def chat(self, reply):
    retrieve = Retrieval_Response()

    while not self.make_exit(reply):
      if retrieve.should_generate_response(reply):
        reply = input(self.generate_response(reply))
      else:
        reply = input(retrieve.respond(reply))
